...
type RequiredRecord<T> = T extends NonNullable<EnvVarKeys>
  ? Record<T[number], string>
  : {};
type OptionalRecord<T> = T extends NonNullable<EnvVarKeys>
...
> =
  | {
      environment: RequiredRecord<RequiredEnv> & OptionalRecord<OptionalEnv>;
      errors: null;
    }
  | { environment: null; errors: string[] };

export interface GenerateEnvironmentOptions<
  Required extends EnvVarKeys,
  Optional extends EnvVarKeys,
> {
  required?: Required;
  optional?: Optional;
}

const getEnvVars = <Fail extends boolean>(
  vars: NonNullable<EnvVarKeys>,
  fail: Fail,
): Fail extends true
...
>({
  required,
  optional,
}: GenerateEnvironmentOptions<RequiredEnv, OptionalEnv>): GenerateEnvironment<
  RequiredEnv,
  OptionalEnv
> => {
